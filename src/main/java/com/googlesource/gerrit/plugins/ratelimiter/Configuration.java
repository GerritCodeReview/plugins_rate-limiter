  ...
  void refreshTable(ProjectConfig newCfg, ProjectConfig oldCfg) {
    if (newCfg != null) {
      try {
        ...
        String oldStringConfig = oldCacheable.get(RATE_LIMITER_CONFIG);
        ImmutableMap<String, String> newCacheable = newCfg.getCacheable().getProjectLevelConfigs();
        String newStringConfig = newCacheable.get(RATE_LIMITER_CONFIG);
        if (oldStringConfig.equals(newStringConfig)) {
          return;
        }
        if (newStringConfig != null) {
...
//  Boolean validateKey(ProjectConfig newCfg, ProjectConfig oldCfg, String key){
//
//    return true;
//  }
