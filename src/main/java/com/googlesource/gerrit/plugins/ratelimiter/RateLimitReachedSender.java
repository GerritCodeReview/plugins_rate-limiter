...
public class RateLimitReachedSender extends OutgoingEmail {
  ...
  protected boolean shouldSendMessage() {
    GroupMembership groupMembership = user.getEffectiveGroups();
    List<UUID> limitRecipients =
        configuration.getRecipients(RateLimitType.UPLOAD_PACK_PER_HOUR_WARN);
    if (!acquirePermit) {
      limitRecipients = configuration.getRecipients(RateLimitType.UPLOAD_PACK_PER_HOUR);
    }
    return groupMembership.containsAnyOf(limitRecipients);
  }
