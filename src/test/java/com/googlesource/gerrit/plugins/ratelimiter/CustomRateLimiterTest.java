...
public class CustomRateLimiterTest {
  ...
  private CustomRateLimiter limiter;
  private ScheduledExecutorService scheduledExecutorMock;

  @Before
  public void setUp() {
    scheduledExecutorMock = mock(ScheduledExecutorService.class);
    RateLimit timeLapse = new RateLimit(RateLimitType.TIME_LAPSE, 1);
    limiter = new CustomRateLimiter(scheduledExecutorMock, RATE, Optional.of(timeLapse));
  }
  ...
  public void testReplenishPermitsIsScheduled() {
    verify(scheduledExecutorMock).scheduleAtFixedRate(any(), eq(1L), eq(1L), eq(TimeUnit.MINUTES));
  }
  ...
  public void testReplenishPermitsScheduledRunnableIsWorking() {
    ...
    verify(scheduledExecutorMock)
        .scheduleAtFixedRate(runnableCaptor.capture(), eq(1L), eq(1L), eq(TimeUnit.MINUTES));
