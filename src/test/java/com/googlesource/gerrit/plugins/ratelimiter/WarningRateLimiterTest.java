...
public class WarningRateLimiterTest {
  ...
  public void setUp() {
    ...
    PeriodicRateLimiter limiter1 =
        spy(new PeriodicRateLimiter(scheduledExecutorMock1, RATE, DEFAULT_TIME_LAPSE_IN_MINUTES, "Any Type"));
    doReturn(1L).when(limiter1).remainingTime(any(TimeUnit.class));

    PeriodicRateLimiter limiter2 =
        spy(new PeriodicRateLimiter(scheduledExecutorMock2, RATE, DEFAULT_TIME_LAPSE_IN_MINUTES, "Any Type"));
  ...
  public void testGetRatePerHour() {
    assertThat(warningLimiter1.maxPermits()).isEqualTo(RATE);
  }
